...
/**
 * This aspect differs from the {@link AbstractOperationCollectionAspect} by the
 * fact that while the {@link Operation} is generated in the <code>before</code>
 * advice, it is added to the collector only if non-
 * <code>null. Otherwise a special
 * &quot;placeholder&quot; is used to signal the <code>after</code> advice
 * <U>not</U> to call {@link OperationCollector} <code>exit</code> methods
 * (since the <code>before</code> advice does not call the
 * {@link OperationCollector#enter(Operation)} if <code>null</code>
 * {@link Operation} is returned by {@link #createOperation(JoinPoint)})
 */
public abstract aspect TrailingAbstractOperationCollectionAspect extends
		OperationCollectionAspectSupport {
	protected static final Operation NULL_PLACEHOLDER = new Operation()
			.type(OperationType.valueOf("null"));
	...
	protected Operation enterOperation(OperationCollector collector,
			Operation op) {
		if (collector != null) {
			collector.enter(op);
			push(op);
	...
	// returns null if {@link OperationCollector#enter(Operation)} should not be
	// called
	...
	after() returning(Object returnValue) : collectionStrategyPoint() {
		KeyValPair<OperationCollector, Operation> kvp = getExitCollector(
				thisJoinPoint, returnValue, null);
	...
	after() throwing(Throwable exception) : collectionStrategyPoint() {
		KeyValPair<OperationCollector, Operation> kvp = getExitCollector(
				thisJoinPoint, null, exception);
		//kvp:getcollector => operation()(pop())
		OperationCollector collector = (kvp == null) ? null : kvp.getKey();
		Operation op = (kvp == null) ? null : kvp.getValue();
		exitOperation(collector, op, null, false, exception);
	}

	protected void exitOperation(OperationCollector collector, Operation op,
			Object returnValue, boolean validReturn, Throwable exception) {
	...
	// returns null if the OperationCollector exit method(s) should not be
	// called
	protected KeyValPair<OperationCollector, Operation> getExitCollector(
			JoinPoint jp, Object returnValue, Throwable exception) {
		return getExitCollector(pop(), jp, returnValue, exception);
	}

	protected KeyValPair<OperationCollector, Operation> getExitCollector(
			Operation op, JoinPoint jp, Object returnValue, Throwable exception) {
		if (op == NULL_PLACEHOLDER) {
			return null; // debug breakpoint
		} else {
			return new KeyValPair<OperationCollector, Operation>(
					getCollector(), op);
		}
	}

	/**
	 * Used to replace a <code>null</code> {@link Operation} returned by the
	 * call to {@link #createOperation(JoinPoint)}
	 */
	 ...
	 *         {@link #NULL_PLACEHOLDER} if the original
	 *         {@link #createOperation(JoinPoint)} returned <code>null</code>
	 */
