...
public aspect HttpClientExecutionCollectionAspect extends
	...
	public pointcut withHttpResponseExecution()
	...
	public pointcut withResponseExecution():withHttpResponseExecution()&&!within(com.ebupt.webjoin..*) 
			&& if(strategies.collect(thisAspectInstance, thisJoinPointStaticPart));
	@SuppressAjWarnings({ "adviceDidNotMatch" })
	before():withResponseExecution(){
		final Operation op = enterOperation(thisJoinPointStaticPart);
		final HttpRequest request = HttpPlaceholderRequest
				.resolveHttpRequest(thisJoinPoint.getArgs());
	...
	@SuppressAjWarnings({ "adviceDidNotMatch" })
	after()returning(HttpResponse response):withResponseExecution(){
	...
	@SuppressAjWarnings({ "adviceDidNotMatch" })
	after()throwing(Throwable exp):withResponseExecution(){
		DefaultOperationCollector collector = (DefaultOperationCollector)getCollector();
		Operation op = collector.builder.peek();
		final HttpRequest request = HttpPlaceholderRequest
				.resolveHttpRequest(thisJoinPoint.getArgs());
		exitOperation(op, request, null, exp);
	}
//	@SuppressAjWarnings({ "adviceDidNotMatch" })
...
//	}
	...
	Object around(ResponseHandler<?> handler) throws IOException
		...
		final AtomicReference<HttpResponse> rspRef = new AtomicReference<HttpResponse>(
				null);
		final ResponseHandler<?> rspHandler = handler;
		// if resolution yielded the placeholder no need to create a wrapper
		ResponseHandler<?> wrapper = (request == HttpPlaceholderRequest.PLACEHOLDER) ? handler
					...
					public Object handleResponse(HttpResponse response)
						...
						if (prevValue != null) {
							throw new IllegalStateException(
									"Multiple responses: current=" + response
											+ "/previous=" + prevValue);
						}
		...
		try {
			...
			if (response == null) {
				throw new IllegalStateException(
						"No response provided during handler invocation");
			}
			exitOperation(op, request, response, null);
			return returnValue;
		} catch (IOException e) {
	...
	Operation exitOperation(Operation op, HttpRequest request,
			HttpResponse response, Throwable e) {
	...
	Operation fillInOperation(Operation op, HttpRequest request,
	...
	OperationMap fillInRequestDetails(OperationMap op, HttpRequest request,
			boolean collectExtra) {
		fillInRequestNetworkDetails(op, request, collectExtra);
		if (collectExtra) {
			fillInMessageHeaders(op.createMap("Request Headers"), request);
		}
		return op;
	}

	OperationMap fillInRequestNetworkDetails(OperationMap op,
			HttpRequest request, boolean collectExtra) {
		RequestLine reqLine = request.getRequestLine();
		op.put("method", reqLine.getMethod()).put(OperationFields.URI,
				reqLine.getUri());
	...
	static String createVersionValue(ProtocolVersion protoVersion) {
		return protoVersion.getProtocol() + "/"
				+ String.valueOf(protoVersion.getMajor()) + "."
				+ String.valueOf(protoVersion.getMinor());
	}

	OperationMap fillInResponseDetails(OperationMap op, HttpResponse response,
			boolean collectExtra) {
		...
		if (collectExtra) {
			...
			fillInMessageHeaders(op.createMap("Response Headers"), response);
		}

		return op;
	}

	OperationMap fillInMessageHeaders(OperationMap headers, HttpMessage msg) {
